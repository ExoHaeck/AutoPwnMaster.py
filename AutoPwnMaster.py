...
while IFS= read -r url; do
    ...
    if echo "$url" | grep -q "TRAVERSAL"; then
        ...
        timeout $TIME_LIMIT dotdotpwn -m http-url -d $DEPTH -f $FILENAME -u "$url" -b -k "$PATTERN" | tee temp_result.txt

        # Revisamos si el resultado contiene 'VULNERABLE'
        if grep -q "VULNERABLE" temp_result.txt; then
            echo "$url <- VULNERABLE" | tee -a $OUTPUT_FILE
        else
            echo "No se encontró vulnerabilidad en: $url"
        fi
